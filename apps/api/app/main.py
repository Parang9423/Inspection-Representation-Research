...
def scan_images() -> int:
    ...
    with connect() as conn:
        for path in DATA_ROOT.rglob("*"):
            ...
            row = conn.execute("SELECT image_id FROM images WHERE path=?", (str(path.resolve()),)).fetchone()
            if row:
                conn.execute("UPDATE images SET filename=?, source_label=?, size_bytes=?, modified_at=?, updated_at=? WHERE path=?", (path.name, source_label, stat.st_size, stat.st_mtime, now, str(path.resolve())))
            else:
                conn.execute("INSERT INTO images (image_id,path,filename,source_label,label,size_bytes,modified_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)", (image_id(path), str(path.resolve()), path.name, source_label, source_label, stat.st_size, stat.st_mtime, now, now))
    return count
...
def list_images(page:int=Query(1,ge=1), page_size:int=Query(48,ge=1,le=200), label:str|None=None, split:SplitCode|None=None, status:StatusCode|None=None, search:str|None=None) -> dict:
    ...
    if split: where.append("split=?"); params.append(split)
...
def bulk_update(request:BulkUpdateRequest)->dict:
    ...
    for key in ["label","split","status","note"]:
        ...
        if value is not None: fields.append(f"{key}=?"); values.append(value)
    for key,value in request.rules.items():
        if key not in RULE_COLUMNS: raise HTTPException(400,f"지원하지 않는 Rule: {key}")
...
def auto_split(request:AutoSplitRequest)->dict:
    if request.train_ratio+request.valid_ratio+request.test_ratio!=100: raise HTTPException(400,"분할 비율의 합은 100이어야 합니다.")
    with connect() as conn:
        query="SELECT image_id,label FROM images WHERE status='included'"+(" AND split='unassigned'" if request.only_unassigned else "")
        rows=conn.execute(query).fetchall(); groups={}
        for row in rows: groups.setdefault(row['label'],[]).append(row['image_id'])
        rng=random.Random(request.seed); assignments=[]; now=datetime.now().isoformat(timespec="seconds")
        for ids in groups.values():
            ...
            assignments.extend(("train",now,v) for v in ids[:train_end]); assignments.extend(("valid",now,v) for v in ids[train_end:valid_end]); assignments.extend(("test",now,v) for v in ids[valid_end:])
...
def next_version(name:str)->str:
    safe=''.join(c for c in name if c.isalnum() or c in '-_') or 'aoi_dataset'; versions=[]
    for path in EXPORT_ROOT.glob(f"{safe}_v*"):
        suffix=path.name.rsplit('_v',1)[-1]
        if suffix.isdigit(): versions.append(int(suffix))
...
def export(request:ExportRequest)->dict:
    ...
    with connect() as conn: rows=conn.execute("SELECT * FROM images WHERE status='included' AND split IN ('train','valid','test')").fetchall()
    manifest=[]; labels=sorted({row['label'] for row in rows})
    for row in rows:
        src=Path(row['path'])
...
def image_file(value:str)->FileResponse:
    with connect() as conn: row=conn.execute("SELECT path FROM images WHERE image_id=?",(value,)).fetchone()
    if not row or not Path(row['path']).exists(): raise HTTPException(404,"이미지를 찾을 수 없습니다.")
    return FileResponse(row['path'])

@app.get("/api/images/{value}/cam")
def cam_file(value:str)->FileResponse:
    with connect() as conn: row=conn.execute("SELECT path FROM images WHERE image_id=?",(value,)).fetchone()
    if not row: raise HTTPException(404,"이미지를 찾을 수 없습니다.")
    source=Path(row['path']); candidates=[CAM_ROOT/source.name]
    try: candidates.insert(0,CAM_ROOT/source.relative_to(DATA_ROOT))
