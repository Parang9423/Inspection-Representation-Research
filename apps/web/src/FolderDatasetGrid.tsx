...
async function fetchFolders() {
  return (await api.get<{ items: FolderItem[] }>('/collaboration/folders')).data
}
...
async function scanDataset() {
  return (await api.post<{ count: number }>('/scan')).data
}
...
export default function FolderDatasetGrid({ worker }: { worker: string }) {
  ...
  const folders = useQuery({
    ...
    refetchInterval: selectedFolder ? false : 5000,
  })
  ...
  const remove = useMutation({
    ...
    onSuccess: async () => {
      setDetail(null)
      await refresh()
      message.success('이미지를 백업 폴더로 이동했습니다.')
    },
      ...
      <>
        ...
        <Card className="toolbar">
          <Space wrap>
            ...
            <Typography.Text type="secondary">폴더를 선택하면 해당 원본 폴더의 이미지만 로드합니다.</Typography.Text>
          </Space>
        </Card>
        {folderItems.length ? (
          <div className="folder-grid">
            {folderItems.map((folder) => {
              const progress = folder.image_count ? Math.round((folder.reviewed_count / folder.image_count) * 100) : 0
              return (
                <Card
                ...
                >
                  ...
                  <Space wrap>
                    ...
                    <Tag color="green">검수 {folder.reviewed_count}</Tag>
                    {folder.reviewing_count > 0 && <Tag color="orange">작업 중 {folder.reviewing_count}</Tag>}
                  </Space>
                  <Typography.Text type="secondary">검수 진행률 {progress}%</Typography.Text>
                  <Button type="link" icon={<FolderOpenOutlined />}>폴더 열기</Button>
                </Card>
              )
            })}
          </div>
        ) : <Empty description="인식된 폴더가 없습니다." />}
      </>
    )
  }
    ...
    <>
      <Card className="toolbar folder-toolbar">
        <Space wrap>
          ...
          <Input.Search
            ...
            style={{ width: 280 }}
          />
          <Typography.Text type="secondary">5초 자동 동기화 · Enter 핫키</Typography.Text>
        </Space>
      </Card>
      ...
      <div className={detail ? 'dataset-review detail-open' : 'dataset-review'}>
        {detail && (
          <aside className="detail-pane">
            <div className="detail-header">
              ...
              <Button onClick={() => void closeDetail()}>닫기</Button>
            </div>
            {detail.locked_by && detail.locked_by !== worker && (
              <Alert type="warning" showIcon icon={<LockOutlined />} message={`${detail.locked_by}님이 검수 중입니다.`} description="조회만 가능하며 수정은 차단됩니다." style={{ marginBottom: 12 }} />
            )}
            ...
            <Card size="small" title="클래스 핫키" className="hotkey-card">
              <div className="hotkey-list">
                {(hotkeys.data?.items ?? []).map((item) => <Tag key={item.key} onClick={() => applyLabel(item.label, true)}>{item.key} · {item.label}</Tag>)}
              </div>
            </Card>
            <Card size="small" title="변경 이력" className="history-card">
              <Table
                size="small"
                pagination={false}
                rowKey="history_id"
            ...
            <Popconfirm
              ...
              description="DB에서 제거되고 원본 파일은 backup_image로 이동합니다."
              okText="이동"
              cancelText="취소"
              disabled={!!detail.locked_by && detail.locked_by !== worker}
              onConfirm={() => remove.mutate({ image_id: detail.image_id, actor: worker, expected_version: detail.version })}
            >
              <Button danger icon={<DeleteOutlined />} block disabled={!!detail.locked_by && detail.locked_by !== worker} loading={remove.isPending}>데이터 삭제(백업 이동)</Button>
            </Popconfirm>
          </aside>
        )}
        ...
        <section className="grid-pane">
                  ...
                  <Card.Meta
                    ...
                    description={(
                      <Space direction="vertical" size={2}>
                        ...
                        <Typography.Text type="secondary">{item.review_status === 'reviewed' ? `검수: ${item.reviewed_by ?? '-'}` : item.review_status}</Typography.Text>
                      </Space>
                    )}
                  />
                </Card>
          ...
          ) : <Empty description="이 폴더에 표시할 이미지가 없습니다." />}
          <Pagination
            current={page}
            pageSize={48}
      ...
      <Modal
        title="클래스 핫키 입력"
        open={hotkeyOpen}
        onCancel={() => { setHotkeyOpen(false); setHotkeyValue('') }}
        onOk={applyHotkey}
        okText="라벨 변경"
        cancelText="취소"
      >
        ...
        <div className="hotkey-modal-list">
          {(hotkeys.data?.items ?? []).map((item) => <Tag key={item.key}>{item.key} · {item.label}</Tag>)}
        </div>
      </Modal>
    </>
  )
